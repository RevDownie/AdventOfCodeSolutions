...
pub fn main() !void {
    ...
    const result_1 = findStartOfPacketMarker(input_file[0..]);
    const result_2 = 0;

    std.debug.print("Part 1: {}, Part 2: {} ms: {}\n", .{ result_1, result_2, @intToFloat(f64, t.read()) / 1000000.0 });
}

/// Look through the data in a sliding window of 4 characters until we find 4 unique characters
/// and return the index+1 of the last character in the sequence
/// data is all lowercase letters
///
fn findStartOfPacketMarker(data: []const u8) usize {
    var i: usize = 0;
